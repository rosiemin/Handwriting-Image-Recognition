...
if __name__ == '__main__':
    ...
    plt.plot(history.history['val_acc'], color= 'navy', linestyle = '-.')
    plt.title('Model accuracy', fontsize=18)
    plt.ylabel('Accuracy',fontsize=16)
    plt.xlabel('Epoch',fontsize=16)
    plt.text(25, 0.80, "Train Accuracy: 0.87\nTest Accuracy: 0.88", fontsize=16)
    plt.legend(['Train', 'Test'], loc='upper left', fontsize=16)
    plt.show()

    # Plot training & validation loss values
    plt.plot(history.history['loss'], color= 'turquoise')
    plt.plot(history.history['val_loss'], color= 'navy', linestyle = '-.')
    plt.title('Model loss', fontsize=18)
    plt.ylabel('Loss',fontsize=16)
    plt.xlabel('Epoch',fontsize=16)
    plt.text(30, 1.1, "Train Loss: 0.32\nTest Loss: 0.43", fontsize=16)
    plt.legend(['Train', 'Test'], loc='upper right', fontsize=16)
    plt.show()
    plot_model(model, to_file='model.png', show_shapes=True)

    plt.plot(history.history['precision'], color = 'turquoise', label = "Train Precision")
    plt.plot(history.history['val_precision'], color = 'navy', label = "Test Precision")
    plt.plot(history.history['recall'],color = 'turquoise', linestyle = '--', label = 'Train Recall')
    plt.plot(history.history['recall'], color = 'navy', linestyle = '--', label = "Test Recall")
    plt.plot(history.history['fbeta'], color = 'turquoise', linestyle = ':', label = 'Train F1 Score')
    plt.plot(history.history['val_fbeta'], color = 'navy', linestyle= ':', label = "Test F1 Score")
