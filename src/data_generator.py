...
chars = ' abcdefghijklmnopqrstuvwxyz'  # noqa
    ...
    def __get_metadata(self):
        ...
        with open(self.fn_path) as f:
            for line in f:
                ...
                self.fn = '{}.png'.format(line_split[0])
                img_path = os.path.join(
                    base_dir, base_words, label_dir,
                    sub_label_dir, self.fn
                )

                # GT text are columns starting at 9
                gt_text = ' '.join(line_split[8:])[:self.max_text_len]

                if any(x.isupper() for x in gt_text):
                    pass
                elif not gt_text.isalpha():
                    pass
                else:
