...
parser.add_argument("-t", "--test_img", help="Test image directory",
                    type=str, default="data/pad_img/test/")
...
for test_img in test_imgs:
    ...
    for i in range(min(len(pred_texts), len(test_img.split("_")[1][0:-4]))):
        if pred_texts[i] == test_img[i]:
            letter_acc += 1
    letter_total += max(len(pred_texts), len(test_img.split("_")[1][0:-4]))

    if pred_texts == test_img.split("_")[1][0:-4]:
        acc += 1
    total += 1
    print('Predicted: %s  /  True: %s' % (pred_texts, test_img.split("_")[1][0:-4]))
